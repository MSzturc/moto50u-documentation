...
![Motorola Edge 50 Ultra](./images/Complete.jpg)
...
- **Photo**
  - **Option 1**  
    If you are looking for the most puristic, DSLR-like experience, I recommend using **AGC9.2.14_V14.0_meitu** together with the **MotoX50Ultra_AGC9.2v2** and **JoqZ92v4rc14** libraries. In this configuration, all lenses are detected correctly and behave as expected. The **triple-lens feature is fully functional**, allowing seamless, continuous zooming between the different lenses during video recording. The influence of the ISP is reduced to a minimum, resulting in a more neutral, DSLR-like image without excessive computational processing or artificial sharpening. **HDR and HDR+ work reliably**, and all special modes such as Slow Motion, Timelapse, Panorama, and others are available. This setup also supports **2× sensor zoom across all lenses**. Autofocus performs reliably, including focus tracking, and the built-in video mode allows recording up to **4K at 60 fps** with the ability to switch lenses on the fly. This is also the only GCam version I have tested that supports **HDR / HDR+ with DCG enabled**. The HDR stack itself is built from multiple frames, with the **first two frames being DCG-based**. These initial DCG frames provide very good noise characteristics, which positively influence the noise behavior of the entire HDR stack. As a result, images appear clean without relying on aggressive noise-reduction algorithms that would otherwise remove fine detail.
  - **Option 2**  
    If you don’t want to invest time in post-processing your photos in Lightroom and prefer a more developed image that can be used straight out of the camera, I recommend **AGC9.1.24_V15.0** together with the **MotoEdge50Ultra (@antykat)** configuration. In this setup, all lenses are detected correctly and behave as expected, however it **does not support a triple-lens configuration**. The influence of the ISP is clearly noticeable, resulting in a more processed image with increased contrast, sharpness, noise reduction, and color correction. Autofocus performs reliably, including focus tracking, and the built-in video mode allows recording up to **4K at 60 fps on all three lenses**. However, switching lenses during a single recording is not possible. While this version lacks support for DCG in HDR / HDR+ mode, the noise-reduction algorithms used here work well and manage to recover a good amount of detail. **HDR and HDR+ function reliably**, and all special modes such as Slow Motion, Timelapse, Panorama, and others are available. Overall, this configuration performs very well if you are looking for a processed image that pops visually, without the need to manually develop the photo yourself.
  - **Option 3**  
    If you still don’t want to invest time in post-processing your photos, but the results from Option 2 feel too processed or artificial and not “silky” enough for your taste, **MotoCam** is still a valid option. Since this is Motorola’s own camera solution, you can expect **all features to work reliably** in both photo and video modes. What makes this option particularly interesting is that MotoCam uses **DCG out of the box for its HDR stack**, which results in very clean images—especially in low-light scenarios where Option 2 can start to look a bit overprocessed. Compared to Option 2, photos look slightly smoother out of the box, with **less sharpening, less contrast, and less aggressive color pop**. I wouldn’t say this makes it better or worse than Option 2—it’s simply a different aesthetic and largely comes down to personal taste. Because of that, it’s worth testing both options to see which look you prefer. Overall, MotoCam is a surprisingly good implementation—at least for **photos**. That said, for **video recording**, it’s not really a good choice compared to the other options discussed in the next section.
- **Video**
...
- **[Case](https://s.click.aliexpress.com/e/_c3uoW7ZB)** - This case forms the foundation of my smartphone setup. In addition to basic protection, it integrates a **MagSafe-compatible ring**, which allows magnetic accessories to be attached securely. For my workflow, it is essential to be able to assemble and disassemble a setup within seconds, which is why I strongly prefer **quick-mount solutions**. Unlike many other cases, the MagSafe ring is **embedded inside the case** rather than glued onto the surface. This prevents the ring from scratching in pockets and avoids lint or dust accumulation around the magnet. Another practical design detail is that the case converts the rounded camera bump into a **more squared-off shape**, which makes it significantly easier to mount and align camera-related accessories.
    ...
    ![Motorola Edge 50 Ultra](./images/case.jpg)
...
- **[Lens Filter Adapter](https://s.click.aliexpress.com/e/_c4MYODYV)** (*optional* but **recommended**)
  ...
  ![Motorola Edge 50 Ultra](./images/filterAdapter1.jpg)
  ...
  ![Motorola Edge 50 Ultra](./images/filterAdapter2.jpg)
  ...
  ![Motorola Edge 50 Ultra](./images/filterAdapter4.jpg)
...
- **[External SSD Case](https://s.click.aliexpress.com/e/_c35GdVX7)** *(**recommended** for RAW recording with MotionCam)*  
  ...
  ![Motorola Edge 50 Ultra](./images/ssdCase.jpg)
  ...
  ![Motorola Edge 50 Ultra](./images/ssdCase2.jpg)
...
![Motorola Edge 50 Ultra](./images/cables.jpg)
